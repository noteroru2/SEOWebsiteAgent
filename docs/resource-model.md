...
| Web        |            0.75 CPU |         512 MB |
| Worker     |            0.50 CPU |         768 MB |
| Total      |            2.00 CPU |        1.75 GB |
...
Source refresh concurrency is one under the heavy-job guard. It processes tracked metadata incrementally and does not persist complete repositories. Individual files are capped at 512 KiB; source-plan context is capped at 6 files, import depth 2, and 40,000 source characters. Source-plan AI concurrency is one. These limits target the same bounded worker envelope.

SERP capture uses the same PostgreSQL heavy-job gate as crawl, AI, source, and GSC work. Each capture launches one Chromium process and one context, caps screenshots at 12,000 pixels high, stores no raw HTML, and closes context and browser in `finally` blocks. Chromium is not kept resident. The 768 MB worker ceiling leaves bounded headroom for the browser while total Compose ceilings remain below half of CX23 RAM.
