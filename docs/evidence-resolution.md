...
Manual SERP observation forms submit a timezone-less local date/time together with an explicit canonical IANA timezone. The server rejects missing, invalid, ambiguous, or nonexistent local times, converts the pair to an absolute UTC instant for `observed_at`, and retains the timezone in `observed_timezone`. The current Thailand pilot displays and submits `Asia/Bangkok`; conversion never depends on browser, server, or container timezone.
